# BLE Protocol Specification: Orange Pi ↔ ESP32-S3 KeyMaster Bridge

## Overview

The ESP32-S3 "KeyMaster" acts as a **bidirectional BLE bridge** between a computer (connected via USB serial) and the Orange Orange Pi 2W (connected via BLE). The Pi runs the Cortex wearable AI companion that stores data in a local SQLite database.

```
Computer ←→ [USB Serial] ←→ ESP32-S3 ←→ [BLE] ←→ Orange Orange Pi 2W
              (CDC/UART)     KeyMaster    (GATT)    (bleak client)
```

## BLE GATT Service

The ESP32 is the **BLE peripheral** (server). The Orange Pi is the **BLE central** (client).
...
| TX Direction | ESP32 → Orange Pi (data from the computer to Pi) |
| RX Characteristic | `a0e1b2c3-d4e5-f6a7-b8c9-0a1b2c3d4e52` |
| RX Properties | WRITE |
| RX Direction | Orange Pi → ESP32 (data from Pi to the computer) |
...
- **Sending (TX notify):** If a message + `\n` exceeds the MTU, the ESP32 should split it across multiple notification events. The Orange Pi buffers incoming bytes and reconstructs messages by splitting on `\n`.
- **Receiving (RX write):** The Orange Pi sends complete messages (including `\n`) in a single `write_gatt_char()` call. BlueZ handles GATT-level fragmentation automatically for writes.

**Important:** The Orange Pi client buffers all incoming notification bytes and only processes complete lines (split on `\n`). Partial messages across notifications are handled correctly.
...
**Computer → Orange Pi (via ESP32 TX):**
```
...
**Orange Pi → Computer (via ESP32 RX):**
```
...
All JSON messages are logged on the Orange Pi. Messages with `"type":"status_request"` trigger an automatic status response.
...
Any message that doesn't start with `{` or `CMD:` is treated as a plain text note. It is saved as a `.txt` file on the Orange Pi and logged.
...
## Orange Pi Client Behavior

For reference, here's what the Orange Pi BLE client does (already implemented):
...
The current KeyMaster firmware **echoes** any data written to RX back via TX notify. This can be used to test the Orange Pi client:
...
[ESP32 forwards via BLE TX]      →  Orange Pi receives, saves note, logs event
[Orange Pi replies via BLE RX]     →  (no reply for notes)

[Computer sends]                 →  CMD:status\n
[ESP32 forwards via BLE TX]      →  Orange Pi receives
[Orange Pi replies via BLE RX]     →  {"type":"status_response","state":"STT_IDLE","disk_free_gb":21.7}\n
[ESP32 forwards via USB serial]  →  Computer receives status JSON

[Computer sends]                 →  Hello from the desktop\n
[ESP32 forwards via BLE TX]      →  Orange Pi saves as text note
```
