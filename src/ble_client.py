...
def _build_discovery_payload():
    """Build DISCOVER: message with Pi's IP and HTTP port.

    Auth uses HTTP Basic Auth with configured username/password,
    so no token needs to be broadcast over BLE.
    """
    if not HTTP_ENABLED:
        return None
    ip = _get_local_ip()
