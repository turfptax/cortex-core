...
def test_file_operations():
    ...
    fid3 = db.insert_file("data.csv", category="uploads",
                          ...
                          project="demo-project", size_bytes=2048)
...
def test_shell_exec():
    ...
    # Basic command
    resp = proto.handle_message('CMD:shell_exec:{"command":"echo hello"}')
    check("shell_exec -> RSP", resp.startswith("RSP:shell_exec:"))
    result = json.loads(resp[15:])
    check("shell_exec exit_code 0", result["exit_code"] == 0)
    check("shell_exec stdout", "hello" in result["stdout"])

    # Command with stderr
    resp = proto.handle_message('CMD:shell_exec:{"command":"echo err >&2"}')
    result = json.loads(resp[15:])
    check("shell_exec stderr", "err" in result["stderr"])

    # Non-zero exit code
    resp = proto.handle_message('CMD:shell_exec:{"command":"exit 42"}')
    result = json.loads(resp[15:])
    check("shell_exec non-zero exit", result["exit_code"] == 42)

    # Missing command field
    resp = proto.handle_message('CMD:shell_exec:{}')
    check("shell_exec no command -> ERR", resp.startswith("ERR:shell_exec:"))

    # Timeout
    resp = proto.handle_message(
        'CMD:shell_exec:{"command":"sleep 0.1 && echo ok","timeout":5}'
    )
    result = json.loads(resp[15:])
    check("shell_exec with timeout", result["exit_code"] == 0)

    # Custom cwd
    resp = proto.handle_message(
        'CMD:shell_exec:{"command":"pwd","cwd":"/tmp"}'
    )
    result = json.loads(resp[15:])
    check("shell_exec cwd", "/tmp" in result["stdout"] or "\\tmp" in result["stdout"].lower())

    db.close()
    os.remove(DB_PATH)
    print("  Shell exec tests complete.")
